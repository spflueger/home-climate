...
// Roger Clark Arduino STM32,
// https://github.com/rogerclarkmelbourne/Arduino_STM32 And stm32duino
...
// RH_ASK on Arduino uses Timer 1 to generate interrupts 8 times per bit
// interval Define RH_ASK_ARDUINO_USE_TIMER2 if you want to use Timer 2 instead
// of Timer 1 on Arduino You may need this to work around other librraies that
// insist on using timer 1 Should be moved to header file #define
// RH_ASK_ARDUINO_USE_TIMER2

// Interrupt handler uses this to find the most recently initialised instance of
// this driver
...
// This is the value of the start symbol after 6-bit conversion and nybble
// swapping
#define RH_ASK_START_SYMBOL 0xb38

RH_ASK::RH_ASK(uint16_t speed, uint8_t rxPin, uint8_t txPin, uint8_t pttPin,
               bool pttInverted)
    : _speed(speed), _rxPin(rxPin), _txPin(txPin), _pttPin(pttPin),
      _rxInverted(false), _pttInverted(pttInverted) {
  // Initialise the first 8 nibbles of the tx buffer to be the standard
  // preamble. We will append messages after that. 0x38, 0x2c is the start
  // symbol before 6-bit conversion to RH_ASK_START_SYMBOL
  uint8_t preamble[RH_ASK_PREAMBLE_LEN] = {0x2a, 0x2a, 0x2a, 0x2a,
                                           0x2a, 0x2a, 0x38, 0x2c};
...
// Put these prescaler structs in PROGMEM, not on the stack
    ...
    (RH_PLATFORM == RH_PLATFORM_GENERIC_AVR8)
...
uint8_t RH_ASK::timerCalc(uint16_t speed, uint16_t max_ticks,
                          uint16_t *nticks) {
#if (RH_PLATFORM == RH_PLATFORM_ARDUINO) ||                                    \
    (RH_PLATFORM == RH_PLATFORM_GENERIC_AVR8)
...
void RH_ASK::timerSetup() {
  ...
  // REVISIT: does not correctly handle 1MHz clock speeds, only works with 8MHz
  // clocks At 1MHz clock, get 1/8 of the expected baud rate
  uint16_t nticks;
  ...
  // If your processor does not have a TCB1, you can change the timer used in
  // RadioHead.h
...
// Clock speed is 48MHz, prescaler of 64 gives a good range of available speeds
// vs precision
...
#define RH_ASK_ZERO_TIMER_IRQ TC3_IRQn
  ...
  // Compute the count required to achieve the requested baud (with 8 interrupts
  // per bit)
  uint32_t rc = (VARIANT_MCK / _speed) / RH_ASK_ZERO_PRESCALER / 8;

  TC->CTRLA.reg |= TC_CTRLA_PRESCALER_DIV64; // Set prescaler to agree with
...
// We use timer 1 TC1_IRQn on TC0 channel 1, since timers 0, 2, 3, 4, 5 are used
...
#define RH_ASK_DUE_TIMER_IRQ TC1_IRQn
  ...
  // Clock speed 4 can handle all reasonable _speeds we might ask for. Its
  // divisor is 128 and we want 8 interrupts per bit
...
#define SYSCORECLOCK 60000000UL // Timer clock tree uses core clock / 2
  ...
  uint16_t prescaler = (uint16_t)(SYSCORECLOCK / 1000000UL) -
                       1; // To get TIM counter clock = 1MHz
  ...
  uint32_t chipkit_timer_interrupt_handler(
      uint32_t currentTime); // Forward declaration
  ...
  esp8266_timer_interrupt_handler(); // Forward declaration
...
bool RH_ASK::send(const uint8_t *data, uint8_t len) {
  ...
  uint8_t count =
      len + 3 + RH_ASK_HEADER_LEN; // Added byte count and FCS and headers to
...
#if (RH_PLATFORM == RH_PLATFORM_GENERIC_AVR8)
  ((value) ? (RH_ASK_TX_PORT |= (1 << RH_ASK_TX_PIN))
...
#endif
#elif (RH_PLATFORM == RH_PLATFORM_ATTINY)
#define RH_ASK_TIMER_VECTOR TIM1_COMPA_vect // TP
...
void RH_INTERRUPT_ATTR esp8266_timer_interrupt_handler() {
  //    timer0_write(ESP.getCycleCount() + 41660000);
  //    timer0_write(ESP.getCycleCount() + (clockCyclesPerMicrosecond() * 100) -
  //    120 );
...
void RH_INTERRUPT_ATTR RH_ASK::receiveTimer() {
  ...
  if (rxSample != _rxLastSample) {
    ...
    _rxPllRamp +=
        ((_rxPllRamp < RH_ASK_RAMP_TRANSITION) ? RH_ASK_RAMP_INC_RETARD
    ...
    if (_rxActive) {
      ...
      if (++_rxBitCount >= 12) {
        ...
        if (_rxBufLen == 0) {
          ...
          // includes the byte count itself, the 4 byte header and the 2 byte
          // FCS
          _rxCount = this_byte;
